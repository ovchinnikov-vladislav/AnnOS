...
void kernel_init_mmap(void)
{
	...
	// LAB3 code here
	// Преобразование физических адресов из структуы kernel_config в виртуальные адреса
	...
	for (int64_t i = state.pages_cnt-1; i >= 0; i--) {
		...
		struct page* p = &state.pages[i];       // сейчас это 32-битная страница
        ...
        // links == 0 - страница используется
		if (links == 0) {
            // p->ref == 1 страница используется ядром или оборудованием, генерируем ошибку
		    assert(p->ref == 1);
		    used_pages++;
		} else {
            memset(p, 0, sizeof(*p));
